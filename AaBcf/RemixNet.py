...
class OSCServer:
    ...
    def sendOSC(self, address=None, msg=None):
        """
        A convenience function so we don't have to dig into the objects
    ...
    def send(self, data):
        ...
        if not data == '':
            try:
                ...
                Live.Base.log('BCF - ERROR: Could not send outgoing data, check your network settings')
                self._outgoing = 0
...
class UDPServer:
    ...
    def bind(self):
        ...
        try:
            ...
            self.bound = 0
            self.parent.log('ERROR: Could not bind to incoming ip/port, check your LiveControl network settings')
